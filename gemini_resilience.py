...
def build_page_prompt(page: GeminiPage, n_files: int) -> str:
    ...
    - summary_data と detail_data の各行には、読んだ箇所が後で追跡できるよう「抽出元テキスト範囲」を短い文字列で入れてください。
    - {simple_quote_rule}
    ...
    - 複数ページ見積の1ページ目にある「防水工事一式」「外部塗装工事一式」「仮設工事一式」などは、detail_data ではなく summary_data.工事項目 に入れてください。
    - 各社のまとめページと明細ページは別物です。まとめページの一式行・諸経費・福利厚生費・端数調整・消費税は summary_data に保持し、明細ページの具体行だけを detail_data に入れてください。
    ...
    {{
      ...
      "summary_data": {{
        ...
        "見積担当": "",
        "工事項目": [
          {{"工事項目": "防水工事", "金額": 11381700, "抽出元テキスト範囲": "工事費内訳 防水工事 11,381,700"}},
          {{"工事項目": "諸経費", "金額": 65000, "抽出元テキスト範囲": "工事費内訳 諸経費 65,000"}},
          {{"工事項目": "厚生福利費", "金額": 55000, "抽出元テキスト範囲": "工事費内訳 厚生福利費 55,000"}}
        ],
        "小計": 11501700,
        "端数調整": -1700,
        "改小計": 11500000,
        "消費税": 1150000,
        "工事費計": 12650000,
        "抽出元テキスト範囲": "小計 11,501,700 / 改小計 11,500,000 / 工事費計 12,650,000"
      }},
      "detail_data": [
        {{"No": 1, "見積元": "瀧上工業", "工事種別": "防水工事", "品名": "平場 ウレタン塗膜防水", "仕様": "X-1工法", "数量": 76.1, "単位": "㎡", "単価": 6400, "金額": 487040, "抽出元テキスト範囲": "平場 ウレタン塗膜防水 X-1工法 76.1㎡ 6,400 487,040"}}
      ]
    }}
    """
